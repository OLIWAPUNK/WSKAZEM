...
func load_saves() -> void:
	...
	for i in range(3):
		var save_file_container = SAVE_FILE_SCENE.instantiate()
		var save_file = save_file_container.get_node("SaveFileButton")
		...
		if saves.has(i):
			...
			save_file.connect("save_file_deleted", _on_save_file_deleted)
		save_file.connect("save_file_selected", func(index: int): save_file_selected.emit(index))
		save_files_container.add_child(save_file_container)

func _on_save_file_deleted(save_file_index: int) -> void:
